...
class EnumSubDomain(object):
    ...
    async def start(self, tasks, tasks_num):
        ...
        :return:
        """
        logger.info('start to brute...')
        for res in self.limited_concurrency_coroutines(tasks, self.coroutine_count):#, bar_format="%s{l_bar}%s{bar}%s{r_bar}%s" % (Fore.YELLOW, Fore.YELLOW, Fore.YELLOW, Fore.RESET), total=tasks_num):
            await res
